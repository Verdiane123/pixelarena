...
const fallback = (name) => ({ default: () => <div className="error-fallback">⚠️ {name} indisponible</div> });

const Header  = lazy(() => import('mfeHeader/Navbar').catch(() => fallback('Header')));
const Lobby   = lazy(() => import('mfeLobby/Lobby').catch(() => fallback('Lobby')));
const Catalog = lazy(() => import('mfeCatalog/Catalog').catch(() => fallback('Catalog')));
const Cart    = lazy(() => import('mfeCart/Cart').catch(() => fallback('Cart')));
